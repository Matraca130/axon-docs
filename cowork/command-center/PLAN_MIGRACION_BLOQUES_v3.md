...
> **Versión:** 3.1 — 2026-03-24 (post-audit)
...
> **Simplificación v3:** Los 11 summaries legacy NO importan. Solo aplica a resúmenes nuevos. No hay "modo legacy", no hay fallback a TipTap monolítico, no hay guard `hasBlocks`. Todo resumen nuevo es block-based desde el día 1.
...
-- Actualizar CHECK constraint de status: drop old + create new
-- Handles both cases: existing constraint or no constraint
DO $$
DECLARE
  cname text;
BEGIN
...
| **~~`hasBlocks` switch~~** | v3: No necesario. Todo resumen nuevo tiene bloques. Se puede simplificar ReaderChunksTab para siempre usar SummaryViewer. |
| **~~Professor `hasBlocks` badge~~** | v3: No necesario. No hay distinción "vista enriquecida" vs legacy. |
...
| **SummaryDetailView** — reemplazar TipTap por BlockEditor (v3: sin switch) | `SummaryDetailView.tsx` | Media | frontend |
...
-- 3. Agregar columnas necesarias (metadata + tracking)
-- NOTE: position_x/y/width/height REMOVED (YAGNI — add when canvas layout is built).
-- Table has 0 rows so future ALTER TABLE is trivial.
ALTER TABLE summary_blocks
...
-- 6. RLS policies — summary_blocks inherits access from summaries
-- NOTE: RLS does NOT auto-inherit via FK. Each table needs its own policies.
ALTER TABLE summary_blocks ENABLE ROW LEVEL SECURITY;

-- Read: anyone who can read the parent summary can read its blocks
CREATE POLICY "sb_read_via_summary" ON summary_blocks
  FOR SELECT USING (
    summary_id IN (SELECT id FROM summaries)
    -- This defers to whatever RLS policies exist on summaries
  );

-- Write: professors/admins who can edit the parent summary
CREATE POLICY "sb_write_via_summary" ON summary_blocks
  FOR ALL USING (
    summary_id IN (
      SELECT id FROM summaries
      -- The summaries table's own RLS handles institution-level access
    )
  );
-- NOTE: If summaries uses more specific RLS (e.g., institution_id checks),
-- replicate that logic here. Run `\dp summaries` to see existing policies.
...
> **⚠️ PREREQUISITO:** Antes de implementar, verificar si `crud-factory.ts` soporta el campo `afterWrite` en su config.
> Correr: `grep -n "afterWrite\|after_write\|onAfterWrite" supabase/functions/server/crud-factory.ts`
> Si NO existe: hay dos opciones:
>   - **Opción A (preferida):** Agregar soporte `afterWrite` al factory (modificar `crud-factory.ts`).
>     En el handler de POST/PUT/DELETE, después del db call, invocar `config.afterWrite?.({ action, row })`.
>   - **Opción B:** Implementar como middleware Hono en `crud.ts`, envolviendo las rutas de summary-blocks.
> **Nota:** "NO tocar crud-factory.ts" en Fase 4 se refiere a no cambiar su lógica CORE (routing, auth, validation). Agregar un hook point es un cambio aditivo seguro.
 ...
 *
 * NOTE: No debounce needed. The dirty-flag operation is idempotent:
 * setting status='review' when already 'review' is a no-op.
 * Serverless functions (Edge Functions) don't persist in-memory state
 * between invocations, so Map-based debounce doesn't work anyway.
 */
...
function flattenBlock(block: BlockRow): string {
  ...
  const s = stripKeywordMarkers; // shorthand — apply to ALL text fields
  switch (block.type) {
    case "prose":
    ...
    // Legacy CMS types (kept for 11 existing summaries)
    case "text":
...
export async function publishSummary(c: Context): Promise<Response> {
  ...
  // 3. Status → published FIRST (embeddings are enhancement, not blocker)
  await db.from("summaries")
    .update({ status: "published" })
    .eq("id", summaryId);

  // 4. autoChunkAndEmbed on the curated content (reuses existing pipeline)
  await autoChunkAndEmbed(summaryId);

  // 5. Per-block embeddings (Opción B) — PARALLEL with concurrency limit
  const CONCURRENCY = 5;
  let blocksEmbedded = 0;
  const embeddableBlocks = blocks.filter(b => {
    const text = flattenBlocksToMarkdown([b]);
    return text.length > 20;
  });

  // Process in batches of CONCURRENCY
  for (let i = 0; i < embeddableBlocks.length; i += CONCURRENCY) {
    const batch = embeddableBlocks.slice(i, i + CONCURRENCY);
    ...
    blocksEmbedded += results.filter(r => r.status === "fulfilled").length;
    // Log failures but don't abort — partial embeddings are OK
    results.filter(r => r.status === "rejected").forEach(r =>
      console.error(`[Publish] Block embedding failed:`, (r as PromiseRejectedResult).reason)
    );
  }

  // 6. Count chunks for response
...
**Opción más limpia (v3 simplificado):** Para resúmenes nuevos, el hook de summaries NO dispara autoChunkAndEmbed. Eso lo hace publish-summary.ts. El hook solo se mantiene activo para los 11 summaries legacy existentes (si alguna vez se editan).
...
```typescript
// ── Block type system ──
// Edu types = all new summaries. Legacy types = 11 old summaries (read-only fallback).
...
// NOTE: position_x/y/width/height removed — not used in v1 (list layout).
// Add via migration when canvas layout is implemented.
...
blocks/CalloutBlock.tsx    — 5 variantes: tip/warning/clinical/mnemonic/exam
...
Componente principal para la edición de resúmenes. No hay fallback a TipTap — todo resumen nuevo usa bloques.
...
│     ├── CalloutForm.tsx       — selector variante + título + contenido
...
**Cambio (v3 simplificado):** Reemplazar TipTapEditor por BlockEditor directamente. No hay switch.
...
Los 11 summaries legacy se ignoran. Todo resumen nuevo usa BlockEditor exclusivamente.
...
src/app/components/student/blocks/CalloutBlock.tsx
...
src/app/components/professor/block-editor/forms/CalloutForm.tsx
...
| ~~`hasBlocks` switch en ReaderChunksTab~~ | v3: no necesario, todo es bloques | — |
...
| Content_markdown se desincroniza de bloques | Baja | Alto | v3: content_markdown se genera SOLO al publicar (una sola vez). No hay edición directa. Si se edita post-publish, status vuelve a 'review' y el content_markdown queda stale hasta re-publish. RPC `check_block_sync_health` detecta esto. |

### 9.3 Invariantes del Sistema

Estas reglas deben cumplirse SIEMPRE después de la migración:

1. **Todo resumen nuevo es block-based.** No hay modo legacy. No hay TipTap monolítico. Los 11 summaries viejos se ignoran.
2. **`content_markdown` es SIEMPRE derivado de bloques.** Se genera SOLO al publicar via flattenBlocksToMarkdown(). Nunca editado directamente.
...
5. **Embedding solo al publicar (v3).** Durante edición (status='review'), NO se genera ningún embedding. onBlockWrite() solo marca el summary como dirty. Embedding ocurre exclusivamente en publish-summary.ts. Esto ahorra costos de API y evita embeddings parciales/inconsistentes.
6. **Si se edita un summary publicado, vuelve a 'review'.** onBlockWrite() detecta status='published' y lo revierte a 'review'. El profesor debe re-publicar para que los embeddings se actualicen. Esto garantiza que RAG siempre tiene contenido consistente.
7. **El profesor siempre usa BlockEditor.** Para resúmenes nuevos, la UI es siempre el editor de bloques. No hay switch ni fallback.
...
| `callout` | `variant` (tip/warning/clinical/mnemonic/exam), `title`, `content` | Destacado temático |
...
IMPORTANTE — Imágenes:
  En producción, imágenes vienen de block.content.image_url (URL de Supabase Storage).
  El prototipo usa block.imageData (base64 inline) — NO copiar eso.
  Los renderers deben usar: <img src={block.content.image_url} alt={block.content.caption} />
  NO leer imageData, imageWidthPx, imagePos del prototipo — esos son solo para el editor.
...
Modificar:
  ...
  supabase/functions/server/summary-hook.ts
    — v3: Para resúmenes nuevos, desactivar autoChunkAndEmbed en el hook
      (publish-summary.ts lo maneja directamente al publicar)
...
NO tocar la lógica core de: auto-ingest.ts, openai-embeddings.ts
PUEDE tocar crud-factory.ts SOLO para agregar soporte afterWrite hook (ver sección 5.1)
...
Modificar:
  src/app/components/roles/pages/professor/SummaryDetailView.tsx
    — Reemplazar TipTap por BlockEditor (v3: sin switch, todo es bloques)
