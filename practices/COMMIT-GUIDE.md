...
> **Propósito:** Workflow obligatorio antes de hacer `git commit` en cualquier repo de Axon.
...
4. [Pre-Commit Checklist — Documentación](#4-pre-commit-checklist--documentación)
...
| `docs` | Solo documentación | `docs: add COMMIT-GUIDE.md` |
...
| `migration` | Migración SQL | `migration: add get_content_tree RPC` |
...
- Descripcion en **minúsculas** (no capitalizar después del type)
- Sin punto final en la primera línea
- Si el commit resuelve un bug conocido: `fix(BUG-XXX): descripción`
- Si el commit es parte de un diagnóstico: `fix(F-XX): descripción` o `fix(O-XX): descripción`
...
- [ ] Archivo no excede límites: componente < 500 líneas, hook < 250, service < 400
```
...
- [ ] Toda ruta LIST tiene paginación con `limit` (MAX 500) y `offset`
...
```markdown
- [ ] Ruta custom < 80 líneas (extraer logica si excede)
- [ ] Route file < 400 líneas (split por dominio si excede)
...
## 4. Pre-Commit Checklist — Documentación
...
- [ ] Si documenta una migración: tiene verification query
- [ ] Si documenta una decisión: usa formato DEC-NNN en `DECISION-LOG.md`
...
| Cambio | Branch? | Razon |
|--------|---------|-------|
| Hotfix de 1 archivo, < 20 líneas | No, directo a `main` | Overhead innecesario para solo-dev |
| Feature nueva o refactor multi-archivo | Si | Poder revertir atomicamente |
| Migración SQL + backend update | Si | Coordinar deploy |
| Solo documentación | No, directo a `main` | No afecta código |
...
src/**/*.md            ← Documentación va en axon-docs
